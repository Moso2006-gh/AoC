const input = `Game 1: 13 red, 18 green; 5 green, 3 red, 5 blue; 5 green, 9 red, 6 blue; 3 blue, 3 green
...
/*let solution = 0;
input.split("\n").forEach((Game, i) => {
    const plays = Game.split(": ")[1].split("; ");
    
    let posible = true;
    plays.forEach(play => {
        const draws = play.split(", ")
        draws.forEach(draw => {
            const [num, color] = draw.split(" ")
            switch (color) {
                case "red":
                    if(Number(num) > 12) posible = false;
                    break;
                case "green":
                    if(Number(num) > 13) posible = false;
                    break;
                case "blue":
                    if(Number(num) > 14) posible = false;
                    break;
                default:
                    break;
            }
...
input.split("\n").forEach((Game, i) => {
    ...
    plays.forEach(play => {
        ...
        draws.forEach(draw => {
            ...
            switch (color) {
                case "red":
                    if(Number(num) > least[0]) least[0] = Number(num);
                    break;
                case "green":
                    if(Number(num) > least[1]) least[1] = Number(num);
                    break;
                case "blue":
                    if(Number(num) > least[2]) least[2] = Number(num);
                    break;
                default:
                    break;
            }
    ...
    solution += least[0] * least[1] * least[2]
})
