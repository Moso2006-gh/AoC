const input = `........................................................862...........20.............453...619......58........694...312.................292.
...
const checkForSimbol = (i, j, matrix) => {
    for (let i_d = -1; i_d <= 1; i_d++) {
        for (let j_d = -1; j_d <= 1; j_d++) {
            try {
                const element = matrix[i + i_d][j + j_d];
                if(element && element !== "." && !/^\d+$/.test(element)) return true;
            } catch (error) {}            
        }
...
matrix.forEach((row, i) => {
    for (let j = 0; j < row.length; j++) {
        if(/^\d+$/.test(row[j])) {
            ...
            for (let e = j; e < row.length; e++) {
                if(!/^\d+$/.test(row[e])) {
                    break;
                }

                numberS += row[e];
                if(canAdd == false && checkForSimbol(i, j, matrix)) canAdd = true;
                j++;
            }

            console.log(numberS, canAdd);
            if(canAdd) sol += Number(numberS)
        }   
    }
})
...
const matrix = input.split("\n").map((line) => line.split(""))
const matrixCheck = matrix.map(row => row.map(t => []))
...
const checkForSimbol = (i, j, matrix) => {
    ...
    for (let i_d = -1; i_d <= 1; i_d++) {
        for (let j_d = -1; j_d <= 1; j_d++) {
            try {
                const element = matrix[i + i_d][j + j_d];
                if(element == "*") cords.push([i + i_d, j + j_d]);
            } catch (error) {}            
        }
    ...
    for (let j = 0; j < row.length; j++) {
        if(/^\d+$/.test(row[j])) {
            ...
            for (let e = j; e < row.length; e++) {
                ...
                numberS += row[e];
                cordsToAddNumber = cordsToAddNumber.concat(checkForSimbol(i, j, matrix))
                j++;
            }

            const filteredCords = [...new Set(cordsToAddNumber.map(x => JSON.stringify(x)))].map(x => JSON.parse(x));
            filteredCords.forEach(cord => {
                console.log(matrixCheck[cord[0], cord[1]]);
                matrixCheck[cord[0]][cord[1]].push(Number(numberS));
            })
        }   
    }
})
...
matrixCheck.forEach(row => {
    row.forEach(element => {
        if(element.length == 2) sol += element[0] * element[1];
    })
})
