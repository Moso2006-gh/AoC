const input = `F|J-F|.|.F-F-7-7F--|77F.7.77F|-L7-|7|-|-F-L7.7FJF-7-JF7.77-F.7F7.FLF7-FF-L7F--F-J-..J-FJ-FLF7.FF.-F7-|-.F7--77F---7-LF7FFL7.F7|7FJ-FL-.-FJ-F
...
const getMovementVectors = (direction) => {
    switch (direction) {
        case "|":
            return [[-1, 0], [1, 0]];
...
for (let m = -1; m <= 1; m++) {
    for (let n = -1; n <= 1; n++) {
        if(Math.sqrt(n**2 + m**2) === 1) {
            try {
                const posibledir = getMovementVectors(matrix[startPos[0] + m][startPos[1] + n]);
                if(posibledir.find((mov => JSON.stringify([-mov[0], -mov[1]]) === JSON.stringify([m, n]))) !== undefined) {
                    posTuCheck.push([startPos[0] + m, startPos[1] + n, 1]);
                }   
            } catch (err) {}
        }
    }
...
const move = (currentPos, posVisited, displacements) => {
    let returnArr = [];
    displacements.forEach(displacement => {
        const newPos = [
            currentPos[0] + displacement[0], 
            currentPos[1] + displacement[1],
            currentPos[2] + 1
        ];
        
        const posibledir = getMovementVectors(matrix[newPos[0]][newPos[1]]);
        try {
            if(posibledir.find((mov => JSON.stringify([-mov[0], -mov[1]]) === JSON.stringify([displacement[0], displacement[1]]))) !== undefined) {
                if(posVisited.find((pos => JSON.stringify([pos[0], pos[1]]) === JSON.stringify([newPos[0], newPos[1]]))) === undefined) {
                    returnArr.push(newPos);
                }
            }
        } catch (error) { } 
    })
...
let LastPosVisited = [[...startPos]];
let posVisited = [[...startPos]]
let maxDis = 0;
while (posTuCheck.length !== 0) {
    let newPosTuCheck = [];
    posTuCheck.forEach(pos => {
        const direction = matrix[pos[0]][pos[1]]; 
        if(direction === undefined) return;

        if(pos[2] > maxDis) maxDis = pos[2];

        let movement = getMovementVectors(direction);

        newPosTuCheck = newPosTuCheck.concat(move(pos, LastPosVisited, movement));
    })

    LastPosVisited = [...posTuCheck];
    posVisited = posVisited.concat([...posTuCheck]);
    posTuCheck = [...newPosTuCheck];
}

let filteredMatrix = matrix.map((line, m) => 
    line.map((el, n) => {
        if(posVisited.find(pos => pos[0] === m && pos[1] === n) !== undefined) {
            return el;
        }  
        else if(el === ".") {
            return "."
...
const filteredInput = filteredMatrix.map(x => x.join("")).join("\n");
...
filteredInput.split("\n").forEach(line => {
    ...
    line.split("").forEach(x => {
        ...
        else newArr[newArr.length - 1] += '.' + x + '.';
    })
    newArr.push(".".repeat(newArr[[newArr.length - 1]].length))
})

newArr.unshift(".".repeat(newArr[[newArr.length - 1]].length));
newArr.push(".".repeat(newArr[[newArr.length - 1]].length));

newArr = newArr.map(x => x.split(""))
    ...
    for (let j = 0; j < newArr[i].length; j++) {
        if(newArr[i][j] !== "." && newArr[i][j] !== "#" ) {
            try {
                switch (newArr[i][j]) {
                    case "|":
                        if(newArr[i - 1][j] !== undefined) newArr[i - 1][j] = "#"
                        if(newArr[i + 1][j] !== undefined) newArr[i + 1][j] = "#"
                        break;
                    case "-":
                        if(newArr[i][j - 1] !== undefined) newArr[i][j - 1] = "#"
                        if(newArr[i][j + 1] !== undefined) newArr[i][j + 1] = "#"
                        break;
...
let spaceToTest = [[0, 0]]
while (spaceToTest.length) {
    let newSpaceToTest = [];
    spaceToTest.forEach(space => {
        newArr[space[0]][space[1]] = " ";
        for (let i = -1; i <= 1; i++) {
            for (let j = -1; j <= 1; j++) {
                try {
                    const testChar = newArr[space[0] + i][space[1] + j] 
                    if(testChar !== undefined) {
                        if(testChar === "." || testChar === "*") {
                            newSpaceToTest.push([space[0] + i, space[1] + j])
                            newArr[space[0] + i][space[1] + j]  = " ";;
                        }
                    }    
                } catch (error) {}
            }
    ...
    spaceToTest = [...newSpaceToTest]
}

console.log('a');

console.log(newArr.map(x => x.join("")).join("\n"));
console.log(newArr.map(x => x.join("")).join("").split("*").length - 1);