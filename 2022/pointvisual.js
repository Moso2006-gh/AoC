...
let sol = '[';
input.split('\n').forEach(line => {
    const pos = line.replace('Sensor at ', '').replace(' closest beacon is at ','').split(':').map(p => p.replace(/[x-y]=/g, '').split(', ').map(x => parseInt(x)));
    const distance = Math.abs(pos[0][0] - pos[1][0]) + Math.abs(pos[0][1] - pos[1][1]);
