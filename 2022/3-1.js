const rucksacksString = `shzsFcPssFhjFssBzdpRcNHNZrpdJdJVJZ
...
rucksacks.forEach(rucksack => {
    const divideIndex = (rucksack.length / 2) - 1;
    let firstGroup = [], secondGroup = [];

    for(let i = 0; i < rucksack.length; i++) {
        if(i <= divideIndex) {
            firstGroup.push(rucksack[i]);
            continue;
        }
        
        if(firstGroup.find(c => c === rucksack[i]) && !secondGroup.find(c => c === rucksack[i])) {

            if(rucksack[i].charCodeAt(0) > 96) {
                priorityScore += rucksack[i].charCodeAt(0) - 96;
            }
            else {
                priorityScore += rucksack[i].charCodeAt(0) - 38;
            }
        }

        secondGroup.push(rucksack[i])
    }
});
