...
for(let i = 0; i*3 + 2 < rucksacks.length; i++) {
    for(char of rucksacks[i*3]) {
        if(rucksacks[i*3+1].includes(char) && rucksacks[i*3+2].includes(char)) {
            if(char.charCodeAt(0) > 96) {
                priorityScore += char.charCodeAt(0) - 96;
            }
            else {
                priorityScore += char.charCodeAt(0) - 38;
            }
