const input = `                                                  ....#.......##.....#......#...#...#.........#.#...................................#.................
                                                  .......#.......#........#............#.......#.....#..............#..............#..................
                                                  ..............#...#.........#................................................#....##..........#..#..
                                                  ..#..#.##.......#..........#....#...##......#............................................#........#.
                                                  .......#...............................#.......#.............#................#......#..............
                                                  .............#................................#.........#.........#.#.....#..................#......
                                                  ....#....................#...#......#...##....................................#..#..#.#.............
                                                  ...............#...............#..#.#.........#...#.............##.....#............................
                                                  ..##.#....#.....#..#..#....#.........#............#...#.#................................#...#......
                                                  ..........#...#..#.....##.#......#...#.........#.....#........#......................#.#............
                                                  ....#......#............#.#........##......#..................#..................#......#...........
                                                  #.......#.............#.......................................#.......#..............##.............
                                                  .............#..............#......##.....#...........#...#.....#........#...#..................#...
                                                  ..................................#.......................#............#.....#.............#........
                                                  ...............#...........#........#.....#..........##...............#.......#....#................
                                                  ......#............................#............................#...............................#...
                                                  ..#...........................##..............#.##...#...........#...................#..#......#.#.#
                                                  ....#...#.......#..........................#...........................#.........#....#..#..........
                                                  ......##........................###............#.#................#......................#..........
                                                  ........#.#..........#...#.......................................#...#..............#...............
                                                  #...............#.#......#.#................#................................#..........#..#........
                                                  ..................#........................#.....................#................................#.
                                                  ......#......................#.......#................................#..#...........#............#.
                                                  .....#.#................##..........................#...#..#....#....#....#.......#.#..#......#.....
                                                  ...#.....#.......#......#..........................##.#.#......#.#....#....##..#......#..#..........
                                                  ................#..#.......#..............#.............#.#.........#....#....##..#......#.........#
                                                  .........###...#..#.#..............#............#..........##........#..#.#.#........#....#.........
                                                  ..................#......#........#...........#.#....#.........................#..#........#........
                                                  ............##.#................#...#..................#..........#........#........................
                                                  ..#........##............................#.............................................#............
                                                  ...........#....#..#......................................#.#...#.##.#...........##....#......#.....
                                                  ...........#.................##.....#..................#....#....#.......#...#...#.#.........##.....
                                                  .....#................#............#...#............#.......#..#.....................#..............
                                                  ...#............#.........................#.....#................#........#.............#..##...#...
                                                  ....................#.............#........................#...........#.#....................#.....
                                                  ........#........#.#.........#..................#........#........#.#...........#.............#.....
                                                  .................................#..#.............#.#...............................#...............
                                                  #......#...#..............#..#...#........#.....#.#..#..............................................
                                                  .....##................#......#..........#.......##........#......#...............#.......#....#....
                                                  ........................#.#...#..............#..............#.......#...#...........#...............
                                                  ...#.#.#..................#...#..#.#.....##............................#........#..#.#..............
                                                  .......................#........##..........#....#.##...#...#...................#...................
                                                  ....#.........#......................#.#..........#...#...................##.........#..............
                                                  ............................#...............................#...#....#....#........#....#..#........
                                                  #..............................#.......................................#....#...................#...
                                                  ..................#.#.......#...#....#...#..................#..........#..........................#.
                                                  .......#......#........#......#....#.................................................#........##....
                                                  ..#...#.#......#.....................#..#........#...............##............##.............#....#
                                                  ......#..#............................................#.................#......#.......#...#..#.....
                                                  ........#..#.....................#....#..........#...##.....#...............#......................#
                                                  ....#............................#.#..............
                                                  ........#......#.............#..........#.........
                                                  .#.#.....................#..........#..#.#......#.
                                                  .....................................#............
                                                  ......#.....#...##.........#..................#...
                                                  .....#................#.....#.......#.............
                                                  ............#..#....#...............###......#....
                                                  ......#........#................#.##....#..#..#...
                                                  ..#............#......#..#........................
                                                  .........#...................#..................#.
                                                  ..##..............................#....#..........
                                                  .........#........................#...............
                                                  .#.................#............#..#.#.....#....#.
                                                  ..................##.................#............
                                                  ............#.#..........##.................#.....
                                                  .....#.#......#..............................##.#.
                                                  ........##..........#..#...#....##.........#....#.
                                                  .....#.....#....#.#...#..#.......#..............#.
                                                  #..............#.........#........................
                                                  .#.......#.#.#.#..............#...........##......
                                                  ....#..#....#.........#...#..............#........
                                                  ........#...#........#...............#.......#....
                                                  ...#...................................#..........
                                                  .............##..#......#.#...#.#.#...............
                                                  ...........#......#....#.....#....................
                                                  ................................#.#....##.........
                                                  ...................#.........................#...#
                                                  ......#.........................................#.
                                                  ....#.................#.#..#.............###......
                                                  .......#...#..#........#....#..........#.#........
                                                  .####..#...#....#.........#.......#.............#.
                                                  .#.......#.#...................##....#...........#
                                                  ....#.#....##.....##.#............#.............#.
                                                  ..................#....###..#...#............#....
                                                  ..........#.................................#.....
                                                  ..#..............#...............#........#.#...#.
                                                  .#.....#...#......#.................#.........#..#
                                                  .....#....#.#..#...#.#........#..#...............#
                                                  ............#............................#.#......
                                                  ......#.........#.......................#.........
                                                  ..........#........#.......................#......
                                                  .............#.................#...........#......
                                                  .......#...........#..#..#....#.............#..#..
                                                  .#....................#......#...........#........
                                                  ........#...#.#........#..........#...............
                                                  ...................#..#.................#.........
                                                  .....#.#..#...#........#....#.........##..###.....
                                                  ...........##.....#.........##......#.............
                                                  .............#..........#....#.......#.#......#...
                                                  ...............#.........#........................
#...............................#......................#..............#...#.....#.#............#....
#................#......#...........#...#.................#.#...##.........#.....#...#............#.
....................#......#..#......##.#...#....................................#.....#.........#..
.............#..#.....#..##..#.........#........#.........................................###.....#.
#.......................#.##.......#............#...........##.......#.................#........#...
...............#....#......#.......#..#....#.....#.....................#.......#....................
.......##................#....#........#..#..#........#.............#...............................
#.....#.................#........#............##.............#..............#...#......#..........#.
..#.......#......#.#........#....#.........................#.#.....#......#.............#...#.......
.........#..#...........................#......................##...............#...#.....#..#.....#
...#..#...#...##.#......#..#.......#...#....#....#.......................#.........#............#.#.
.#......................#...#................#.#........................#..........#.#.....#.....#..
............................#...#..........#.......#.....#............#.....#...#...#..#..#.........
..#..........#..........#..#.#..##.....................#.......#.....................#.......#......
...#...#.........................#......#.#...............#.........#........................##.....
..............#......#...................#..#..................................#.....###.........#..
.#.#.........#..#........#..........#.........................#....#...........#.#.......#........#.
#..........#............................................#..............##...#...........#...........
....#.#..................#.#.#.............#.#...#.......#....#....................................#
#..#.........#.......................................#.......................#...................#.#
...............#.....#..............#...##...........#..........#.....................#.............
...#...#.................#.#......#.#....#...............#.............................#.....#......
............#...#...#.....#.....#...................#..#..............#..............#..............
...............................#........#..............#...#...#........#.#..........##...#..#..#...
........#.............#..........#..............#.......#..............................#............
..........#..#.................................#..............#.#....###..#..#....#.........#.......
.....................#....#............#....................#.........#...#...#....#........#......#
.#.#.#..........##...........................#.#.............#..............#.................#.....
..........#........................#...................................#.......#....#.........#.....
...................#...#..#........#.#.#.........#................#..#.#.............#..............
..#........#.......#...............#.......#...#......#..........#.....#............#...............
........#..#.................#..#.#......#......#...#...........#.......#..#.....................#..
.....................#.....#...........#......#...........#..#...#.....#......#................#....
..........#......#.#..#.#..........................#..............#.........#.#................#..#.
............#...............................#....#.....................#.....#.......#....##.......#
................................#.............#...................#.....#..#...#.......#............
...........#..........#.#......#......#.#..#.........#..........#..#.#........#.......#......#.....#
..#.............................#..#......#.......#.#.#........#....................................
......................................#.........#............#.........#.........#...............##.
........#............#............#.#...#.#........##......#...#........#........#................#.
...................#....................#.........#....#..#.........#.....#........#.......#........
.#....#....#...........................#......#.........................#.....#.......#.............
...#..............................#........#.................#........#...........................#.
#.......#....#........#......#...#....#......................#..............................#.......
........##......#.......#....................#....................#.#.#................#..#.......#.
..........#............#............#.......................#..#..#...........#.......#.#........#..
#.....................................#.#............#.........#.......#.....##..................##.
..........#..............#..#......#.................##..................#..................#..#....
....#..............#.#.......#.........#.........................#..................#....#...#......
............................................#..#..#..#........#.........#.#......#.......#.....#....
....####.....#..............#......#.....#.....#..
....#....#.................#..#.......##..........
.##.......#......#..........#.#.....#.............
#........#...........#..........#..............##.
......##......##......##....#......#.............#
#................#...............#..#.............
.................#.....................#..#.......
.....#.#.....#................#....#..............
#.##......#..##.........#.........#...#...........
...##..#.........#......#.........##..............
.#......#......#......#..................#........
...#....##..#......#.................#...........#
................................##.....#..#....#..
#............#....#.......#.............#...#....#
..........#.......................................
.....#..............................#.....#...#.#.
........#.........#...##..#...............#.......
#........#.#..............#..#.......#..........#.
...#...............#....#...##.............#......
#...........#............................#........
......##...#.........#....#.....##..#...........#.
...#.#..#................#....................#..#
.....#....###........#.......#......#.......#.....
............................#.#.........#.........
......#.#.........#...#............#......#..#....
......#..#.#...#.................##............#..
...#....#.................#............##....#....
........#........................................#
..#.............#.##..............................
.#..............#..................#......#..#....
.#..#.............##...........##.........##......
..#....##...............#............#....##......
........................#.........................
............#...................#.................
.#..........#..........#..........#.#.............
........#.#................................#...#..
..........#......#........#.....#.#..#....##...#..
...........#.#..........................#....##...
...............................#...#.#............
.#.............#.................#.#.......#..#..#
....#................#...#............#...........
.........#...##......#..#.........................
...#.#....#.........................#.#...........
....#......##......##.....##..................#...
.#.......#.......#..#.......##....#.........#..#.#
..#...##.................#........................
.#.#..#.................#....#................#...
.....#...................#............#.......##..
.#...#......#.#...#..........#..........#...#.#...
..#.......#..#.#......#..........#.....#.........#


`

const n = 50;

let [matrixString, directions] = input.split('\n\n');
directions = directions.slice(0, -1);

let matrix = [];
matrixString.split('\n').forEach(line => {
    matrix.push([]);
    for (let i = 0; i < line.length; i++) {
        const chr = line[i]
        
        if(chr === ' ') continue;
        let rock = chr === '.' ? 0 : 1;
        matrix[matrix.length - 1].push([i, rock])        
    }
})

const originalPos = [0, matrix[0][0][0]];
let dir = 0;
let number = '';
for (let i = 0; i < directions.length; i++) {
    const cmdS = directions[i];
    if(cmdS === 'R' || cmdS === 'L' || i === directions.length - 1) {
        if(i === directions.length - 1) {
            number += cmdS;
        }
        const cmd = parseInt(number);
        let state;
        let stop = false;
        for (let i = 0; i < cmd; i++) {
            if(stop) break;
            const currentFace = [Math.floor(originalPos[0] / n), Math.floor(originalPos[1] / n)]
            switch (dir) {
                case 0:
                    state = matrix[originalPos[0]][originalPos[1] + 1 - matrix[originalPos[0]][0][0]]; 
                    if(state !== undefined) {
                        if(!state[1]) originalPos[1]++;
                        else stop = true;
                    }
                    else {
                        const yDisplacement = originalPos[0] - n * currentFace[0];
                        if(currentFace[0] === 0 ) /*Revisado*/ {
                            const newPos = matrix[(3*n - 1) - yDisplacement][matrix[(3*n - 1) - yDisplacement].length - 1];
                            if(!newPos[1]) { 
                                originalPos[0] = (3*n - 1) - yDisplacement;
                                originalPos[1] = newPos[0];
                                dir = 2;
                            }
                            else stop = true;
                        }
                        if(currentFace[0] === 1 ) /**Revisado */ {
                            const newPos = matrix[n - 1][yDisplacement + n];
                            if(!newPos[1]) { 
                                originalPos[0] = n - 1;
                                originalPos[1] = newPos[0];
                                dir = 3;
                            }
                            else stop = true;
                        }
                        if(currentFace[0] === 2 ) /**Revisado */ {
                            const newPos = matrix[(n - 1) - yDisplacement][matrix[(n - 1) - yDisplacement].length - 1];
                            if(!newPos[1]) { 
                                originalPos[0] = (n - 1) - yDisplacement;
                                originalPos[1] = newPos[0];
                                dir = 2;
                            }
                            else stop = true;
                        }
                        if(currentFace[0] === 3 ) /**Revisado */ {
                            const newPos = matrix[(3 * n - 1)][yDisplacement + n];
                            if(!newPos[1]) { 
                                originalPos[0] = (3 * n - 1);
                                originalPos[1] = newPos[0];
                                dir = 3;
                            }
                            else stop = true;
                        }
                    }      
                    break;

                case 1:
                    try{state = matrix[originalPos[0] + 1][originalPos[1] - matrix[originalPos[0] + 1][0][0]];} catch{state = undefined} 
                    if(state !== undefined) {
                        if(!state[1]) originalPos[0]++;
                        else stop = true;
                    }
                    else {
                        const xDisplacement = originalPos[1] - n * currentFace[1];

                        if(currentFace[1] === 0) /**Revisado */ {
                            const newPos = matrix[0][xDisplacement + n];
                            if(!newPos[1]) { 
                                originalPos[0] = 0;
                                originalPos[1] = newPos[0];
                                dir = 1;
                            }
                            else stop = true;
                        }
                        if(currentFace[1] === 1) /**Revisado */ {
                            const newPos = matrix[3*n + xDisplacement][49];
                            if(!newPos[1]) { 
                                originalPos[0] = 3*n + xDisplacement;
                                originalPos[1] = newPos[0];
                                dir = 2;
                            }
                            else stop = true;
                        }
                        if(currentFace[1] === 2) /**Revisado */ {
                            const newPos = matrix[n + xDisplacement][49];
                            if(!newPos[1]) { 
                                originalPos[0] = n + xDisplacement;
                                originalPos[1] = newPos[0];
                                dir = 2;
                            }
                            else stop = true;
                        }
                    }
                    break;
                case 2:
                    state = matrix[originalPos[0]][originalPos[1] - 1 - matrix[originalPos[0]][0][0]]; 
                    if(state !== undefined) {
                        if(!state[1]) originalPos[1]--;
                        else stop = true;
                    }
                    else {
                        const yDisplacement = originalPos[0] - n * currentFace[0];
                        if(currentFace[0] === 0) /**Revisado */ {
                            const newPos = matrix[(3* n - 1) - yDisplacement][0];
                            if(!newPos[1]) { 
                                originalPos[0] = (3* n - 1) - yDisplacement;
                                originalPos[1] = newPos[0];
                                dir = 0;
                            }
                            else stop = true;
                        }
                        if(currentFace[0] === 1) /**Revisado */ {
                            const newPos = matrix[2*n][yDisplacement];
                            if(!newPos[1]) { 
                                originalPos[0] = 2*n;
                                originalPos[1] = newPos[0];
                                dir = 1;
                            }
                            else stop = true;
                        }
                        if(currentFace[0] === 2) /**Revisado */ {
                            const newPos = matrix[(n - 1) - yDisplacement][0];
                            if(!newPos[1]) { 
                                originalPos[0] = (n - 1) - yDisplacement;
                                originalPos[1] = newPos[0];
                                dir = 0;
                            }
                            else stop = true;
                        }
                        if(currentFace[0] === 3) /**Revisado */ {
                            const newPos = matrix[0][yDisplacement];
                            if(!newPos[1]) { 
                                originalPos[0] = 0;
                                originalPos[1] = newPos[0];
                                dir = 1;
                            }
                            else stop = true;
                        }
                    }
                    break;
                case 3:
                    try{state = matrix[originalPos[0] - 1][originalPos[1] - matrix[originalPos[0] - 1][0][0]]} catch {state = undefined}

                    if(state !== undefined) {
                        if(!state[1]) originalPos[0]--;
                        else stop = true;
                    }
                    else {
                        const xDisplacement = originalPos[1] - n * currentFace[1];
                        if(currentFace[1] === 0) /**Revisado */ {
                            const newPos = matrix[xDisplacement + n][0];
                            if(!newPos[1]) { 
                                originalPos[0] = xDisplacement + n;
                                originalPos[1] = newPos[0];
                                dir = 0;
                            }
                            else stop = true;
                        }
                        if(currentFace[1] === 1) /**Revisado */ {
                            const newPos = matrix[3*n + xDisplacement][0];
                            if(!newPos[1]) { 
                                originalPos[0] = 3*n +  xDisplacement;
                                originalPos[1] = newPos[0];
                                dir = 0;
                            }
                            else stop = true;
                        }
                        if(currentFace[1] === 2) /**Revisado*/ {
                            const newPos = matrix[matrix.length - 1][xDisplacement];
                            if(!newPos[1]) { 
                                originalPos[0] = matrix.length - 1;
                                originalPos[1] = newPos[0];
                                dir = 3;
                            }
                            else stop = true;
                        }
                    }
                    break;
            }   
        }
        if(i !== directions.length - 1) {
            dir = cmdS === 'R' ? (dir + 1) % 4 : (dir - 1) >= 0 ? dir - 1 : 3  
            number = '';
        }
    }
    else number += cmdS
}

console.log((originalPos[0] + 1) * 1000 + (originalPos[1] + 1) * 4 + dir);